<?php
final class Modelos_Constructora_Contratos extends Modelo {
	protected $_db = null;
	...
	public function listadoContratos($id = null) {
		try {
			$sth = $this->_db->query("
				SELECT cc.id, p.nombre, p.lote, p.manzana, p.seccion
				FROM constructora_contratos cc
				JOIN propietarios p
				WHERE p.id = cc.id_contratante
				ORDER BY cc.id DESC
			");
			if(!$sth->execute()) throw New Exception();
			...
			while ($datos = $sth->fetch()) {
				...
				if ($id == $datos['id']) {
					...
					$html .= '<option value="' . $datos['id'] . '">' . $datos['id'] . ': ' . $datos['nombre'] . ' [' . $prefijo . '-' . $datos['manzana'] . '-' . $datos['lote'] . ']</option>';
				}
			}
