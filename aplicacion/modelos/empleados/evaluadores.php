<?php
final class Modelos_Empleados_Evaluadores extends Modelo {
	protected $_db = null;
	...
	public function nuevo($datos) {
		try {
			...
			$autorizar_proveedores = $datos['autorizar_proveedores'];
			...
			$arregloDatos = array($nombre, $apellidos, $puesto, $email, $telefono, $celular, $evaluador, $salt, $contrasenaEncriptada, $nombreUsuario, $contrasena, $autorizar_proveedores);

			$sth = $this->_db->prepare("INSERT INTO empleados (nombre, apellidos, puesto, email, telefono, celular, evaluador, salt, contrasena, tipo, nombreUsuario, num, autorizar_proveedores) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 3, ?, ?, ?)");
	...
	public function modificarGuardar($datos) {
		try {
			...
			$arregloDatos = array($nombre, $apellidos, $puesto, $email, $telefono, $celular, $evaluador, $id_jefe, $nombreUsuario, $autorizar_proveedores, $id_departamento, $id_centro_trabajo, $id_puesto, $id);
	...
	public function inactivar($id) {
		try {
			...
			$sth->bindParam(1, $id);
			if(!$sth->execute()) throw New Exception();
	  		header('Location: ' . STASIS . '/empleados/evaluadores');
		} catch (Exception $e) {
			$this->mensajes[] = Modelos_Sistema::status(0, $e->getMessage());
		}
	}
