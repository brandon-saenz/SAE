<?php
final class Modelos_Empleados_Ejecutivos extends Modelo {
	protected $_db = null;
	...
	public function listado() {
		try {
			...
			// Activos
			$sth = $this->_db->query("
				SELECT e.id, e.nombre, e.apellidos, e.email, e.telefono
				FROM empleados e
				WHERE e.tipo = 8 AND e.status = 1
	...
	public function nuevo($datos) {
		try {
			...
			$sth = $this->_db->prepare("INSERT INTO empleados (nombre, apellidos, puesto, email, telefono, celular, salt, contrasena, tipo, nombreUsuario, num, id_departamento, id_centro_trabajo, id_puesto) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 8, ?, ?, ?, ?, ?)");
			if($sth->execute($arregloDatos)) {
				...
				$this->mensajes[] = Modelos_Sistema::status(2, 'Ejecutivo agregado exitosamente.');
			} else {
				throw New Exception();
			}
		} catch (Exception $e) {
	...
	public function modificarGuardar($datos) {
		try {
			...
			$sth = $this->_db->prepare("UPDATE empleados SET
										...
										WHERE id = ?");
			if($sth->execute($arregloDatos)) {
				$this->mensajes[] = Modelos_Sistema::status(2, 'Ejecutivo modificado exitosamente.');
			} else {
				throw New Exception();
			}
		} catch (Exception $e) {
	...
	public function inactivar($id) {
		try {
			...
			$sth->bindParam(1, $id);
			if(!$sth->execute()) throw New Exception();
	  		header('Location: ' . STASIS . '/empleados/ejecutivos');
		} catch (Exception $e) {
			$this->mensajes[] = Modelos_Sistema::status(0, $e->getMessage());
		}
	}
