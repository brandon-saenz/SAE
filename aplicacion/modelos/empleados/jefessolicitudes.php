<?php
final class Modelos_Empleados_JefesSolicitudes extends Modelo {
	protected $_db = null;
	...
	public function nuevo($datos) {
		try {
			...
			$sth = $this->_db->prepare("INSERT INTO empleados (nombre, apellidos, puesto, email, telefono, celular, salt, contrasena, tipo, nombreUsuario, num, id_departamento, id_centro_trabajo, id_puesto) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 5, ?, ?, ?, ?, ?)");
	...
	public function inactivar($id) {
		try {
			...
			$sth->bindParam(1, $id);
			if(!$sth->execute()) throw New Exception();
	  		header('Location: ' . STASIS . '/empleados/jdsolicitudes');
		} catch (Exception $e) {
			$this->mensajes[] = Modelos_Sistema::status(0, $e->getMessage());
		}
	}
	...
	public function listadoJefes($idEmpleado = null) {
		try {
			if (isset($idEmpleado)) {
	...
	public function listadoCobranza($idEmpleado = null) {
		try {
			if (isset($idEmpleado)) {
				$sth = $this->_db->prepare("SELECT id_jefe FROM empleados WHERE id = ?");
				$sth->bindParam(1, $idEmpleado);
			...
			$sth = $this->_db->query("
				SELECT e.id, CONCAT(e.nombre, ' ', e.apellidos) AS jefe
				FROM empleados e
				JOIN puestos p
				ON p.id = e.id_puesto
				WHERE e.status = 1 AND e.tipo = 5 AND p.nombre = 'GERENTE COBRANZA'
				ORDER BY e.nombre ASC, e.apellidos ASC
			");
			if(!$sth->execute()) throw New Exception();
