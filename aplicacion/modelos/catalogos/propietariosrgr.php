<?php
final class Modelos_Catalogos_PropietariosRgr extends Modelo {
	protected $_db = null;
	...
	public function listado() {
		try {
			...
			while ($datos = $sth->fetch()) {
	  		...
	  		// Inactivos
			$sth = $this->_db->query("
				SELECT id, seccion, manzana, lote, nombre, email, telefono1, telefono2, superficie, nombreUsuario
				FROM propietarios
				WHERE tipo = 'RGR' AND status = 0
				ORDER BY nombre ASC
			");
			if(!$sth->execute()) throw New Exception();
	...
	public function inactivar($id) {
		try {
			...
			$sth->bindParam(1, $id);
			if(!$sth->execute()) throw New Exception();
	  		header('Location: ' . STASIS . '/catalogos/propietariosrgr');
		} catch (Exception $e) {
			$this->mensajes[] = Modelos_Sistema::status(0, $e->getMessage());
		}
	}
	...
	public function generarCc() {
		try {
			...
			$correo->claveCatastral($id);
	  		...
	  		header('Location: ' . STASIS . '/catalogos/propietariosrgr/cc/' . $id . '/1');
		} catch (Exception $e) {
			$this->mensajes[] = Modelos_Sistema::status(0, $e->getMessage());
		}
	}
