<?php
final class Modelos_Catalogos_Inventario extends Modelo {
	protected $_db = null;
	...
	public function listado() {
		try {
			...
			// Activos
			$sth = $this->_db->query("
				SELECT i.id, i.nombre, a.nombre AS arrendadora, i.disponible, i.m2, i.importe_renta
				FROM inventario i
				JOIN arrendadoras a
				ON a.id_arrendadora = i.id_arrendadora
				ORDER BY id DESC
			");
			...
			while ($datos = $sth->fetch()) {
				switch ($datos['disponible']) {
					case 'S': $disponible = 'DISPONIBLE'; break;
					case 'N': $disponible = 'VENDIDO'; break;
				...
				$arreglo = array(
					...
					'm2' => number_format($datos['m2'], 2, '.', ','),
					'importe_renta' => '$ ' . number_format($datos['importe_renta'], 2, '.', ','),
				);
