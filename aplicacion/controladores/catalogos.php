...
final class Catalogos extends Controlador {
	...
	function propietariosrgr($accion = null, $id = null, $status = null) {
		...
		switch ($accion) {
			...
			case 'pago':
			$pagina->set('titulo', 'Aplicar Pago(s)');

			if (!empty($status)) $pagina->set('status', Modelos_Sistema::status(2, 'Propietario agregado.'));
			// $pagina->set('datos', $propietarios->expediente($id));
			$pagina->set('pago', 1);
			break;
	...
	function cobroplan($accion = null, $id = null, $status = null) {
		...
		switch ($accion) {
			...
			case 'reporte':
			$pagina->set('titulo', 'Reporte de Movimientos Diarios');
			$pagina->set('reporte', 1);
			break;
	...
	function proveedores($accion = null, $id = null, $status = null) {
		...
		switch ($accion) {
			...
			case 'contrato':
			$pagina->set('titulo', 'Cargar Contrato de Proveedor');

			if (!empty($status)) $pagina->set('status', Modelos_Sistema::status(2, 'Contrato del proveedor cargado.'));

			$pagina->set('contrato', 1);
			$pagina->set('info', $proveedores->getInformacion($id));
			break;
	...
	function inventario($accion = null, $id = null, $status = null) {
		...
		switch ($accion) {
			...
			case 'modificar':
			$pagina->set('titulo', 'Editar Lote');

			if (!empty($status)) $pagina->set('status', Modelos_Sistema::status(2, 'Lote actualizado.'));
			...
			case 'nuevo':
			$pagina->set('titulo', 'Nuevo Lote');

			if (!empty($status)) $pagina->set('status', Modelos_Sistema::status(2, 'Lote agregado.'));

			$pagina->set('nuevo', 1);
			break;

			default:
			$pagina->set('titulo', 'Inventario de Lotes');
	...
	function contratos($accion = null, $id = null, $status = null) {
		...
		$contratos->nuevo();
		if (isset($_POST['modificarGuardar']))
		$contratos->modificarGuardar();

		switch ($accion) {
			case 'cancelar':
			$contratos->cancelar($id);
			break;
			...
			if (!empty($status)) $pagina->set('status', Modelos_Sistema::status(2, 'Contrato agregado.'));

			$pagina->set('nuevo', 1);
			$pagina->set('listadoLotes', $inventario->listadoLotes());
			$pagina->set('listadoVendedores', $procesos->listadoUsuariosVentas());
			break;
	...
	function masterplan($accion = null, $id = null, $status = null) {
		$acceso = $this->cargarModelo('acceso');
		$masterplan = $this->cargarModelo('catalogos_masterplan');

		!$acceso->estaLoggeado()? $pagina = $this->redireccionar('./') : $pagina = $this->cargarVista('catalogos/masterplan');

		if (isset($_POST['nuevo']))
		$masterplan->nuevo($_POST);
		if (isset($_POST['modificarGuardar']))
		$masterplan->modificarGuardar($_POST);

		switch ($accion) {
			case 'inactivar':
			$masterplan->inactivar($id);
			break;

			case 'reactivar':
			$masterplan->reactivar($id);
			break;
			...
			case 'nuevo':
			$pagina->set('titulo', 'Cargar Master Plan');

			if (!empty($status)) $pagina->set('status', Modelos_Sistema::status(2, 'Archivo agregado.'));

			$pagina->set('nuevo', 1);
			break;

			default:
			$pagina->set('titulo', 'Historial de Master Plan');
			$pagina->set('listado', $masterplan->listado());
			break;
		}

		if (!empty($masterplan->mensajes)) $pagina->set('mensajes', $masterplan->mensajes);
		$pagina->set('estaLoggeado', $acceso->estaLoggeado());
		$pagina->set('menu1', 'catalogos');
		$pagina->set('menu2', 'postventa');
		$pagina->set('menu3', 'masterplan');
		$pagina->renderizar();
	}
