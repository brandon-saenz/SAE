...
final class Modelos_Solicitudes extends Modelo {
	...
	public function generar() {
		try {
			...
			$arregloDatos = array($idPropietario, $tipo, $id_servicio, $otro, $descripcion, $uniqueId);
			$sth = $this->_db->prepare("INSERT INTO solicitudes (id_propietario, fecha_creacion, tipo, id_servicio, otro, descripcion, uniqueid) VALUES (?, NOW(), ?, ?, ?, ?, ?)");
		...
		if ($cComentarios >= 1) {
			...
			while ($datos = $sth->fetch()) {
				...
				if (!$datos['usuario']) {
					...
					$fotoComentario = '<img src="https://saevalcas.mx/' . $foto . '" height="30" />';
			...
			if (!empty($responsable)) {
				...
				$htmlResponsable = '
					...
					<table>
						<tr>
							<td style="width: 15%">
								<img src="https://saevalcas.mx/' . $foto . '" height="55" />
							</td>
							<td style="width: 85%">
		...
		try {
			...
			if ($tipo == 'enviadas') {
				$sth = $this->_db->prepare("
					...
					LEFT JOIN servicios se
					ON se.id = so.id_servicio
					WHERE so.id_propietario = ? AND (so.status = 0 OR so.status = 9)
					ORDER BY so.id DESC
				");
	...
	public function info() {
		try {
			...
			if (!$datos['foto']) {
				...
				$foto = 'https://saevalcas.mx/data/f/' . $datos['foto'];
			}
			$conclusion = $datos['conclusion'];
			$administrador = $datos['administrador'];
			$fecha_atendida = $datos['fecha_atendida'];
			$fechaCreacion = Modelos_Fecha::formatearFechaHora($datos['fecha_creacion']);
			$qr = '<img src="http://chart.apis.google.com/chart?cht=qr&chs=300x300&chl=https://saevalcas.mx/movimientos/solicitudes/visualizar/' . $uniqueId . '&chld=H|0" width="50%" style="margin-bottom: 13px;"><br />';
			...
			if (!$datos['responsable']) {
				$html = '
					<div class="col-md-12 text-center">
						...
						' . $qr . '
						<a target="_blank" href="https://saevalcas.mx/movimientos/solicitudes/visualizar/' . $uniqueId . '" class="btn btn-primary mr-2" style="background: #83AB29; border: 1px #83AB29 solid;"><i class="fa fa-download"></i> Descargar PDF</a>
					</div>
					<div class="col-md-12">
